...
if ! [ -e /root/install/installed.ans ]; then
	...
	if [ $# -eq 0 ]; then
	    /opt/kaltura/bin/kaltura-base-config.sh /root/install/default.config.ans
	    /opt/kaltura/bin/kaltura-nginx-config.sh /root/install/default.config.ans
	else
        /opt/kaltura/bin/kaltura-base-config.sh $1
        /opt/kaltura/bin/kaltura-nginx-config.sh $1
	fi
